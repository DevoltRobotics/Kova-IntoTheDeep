...
public class PinpointDrive extends MecanumDrive {
    ...
    public static Params Pinpoint_PARAMS = new Params();
    public GoBildaPinpointDriverRR pinpoint;
    private Pose2d lastPinpointPose = pose;

    public PinpointDrive(HardwareMap hardwareMap, Pose2d pose) {
        super(hardwareMap, pose);
        FlightRecorder.write("PINPOINT_PARAMS", Pinpoint_PARAMS);
        pinpoint = hardwareMap.get(GoBildaPinpointDriverRR.class, Pinpoint_PARAMS.pinpointDeviceName);

        if (Pinpoint_PARAMS.usePinpointIMUForTuning) {
            lazyImu = new LazyImu(hardwareMap, Pinpoint_PARAMS.pinpointDeviceName, new RevHubOrientationOnRobot(zyxOrientation(0, 0, 0)));
        }

        // RR localizer note: don't love this conversion (change driver?)
        pinpoint.setOffsets(DistanceUnit.MM.fromInches(Pinpoint_PARAMS.xOffset), DistanceUnit.MM.fromInches(Pinpoint_PARAMS.yOffset));


        pinpoint.setEncoderResolution(Pinpoint_PARAMS.encoderResolution);

        pinpoint.setEncoderDirections(Pinpoint_PARAMS.xDirection, Pinpoint_PARAMS.yDirection);
